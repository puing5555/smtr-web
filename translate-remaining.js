...
const translations = {
  ...
  '"Canton\'s price plunge just before the halving. Is this a ripple effect?"': '"반감기 직전 캔톤 가격 폭락. 이것은 연쇄 반응인가?"',
...
for (const [eng, kor] of Object.entries(translations)) {
  if (c.includes(eng)) {
    c = c.split(eng).join(kor);
    count++;
  }
}

fs.writeFileSync(path, c, 'utf8');
console.log('Replaced:', count, 'unique titles');
