#!/usr/bin/env python3
"""
V10.1 프롬프트 자율 개선 - DB 시그널 분석 (REST API 방식)
"""
import requests
import json
import re
...
def fetch_all_signals():
    ...
    try:
        ...
        if signals_response.status_code != 200:
            print(f"시그널 조회 실패: {signals_response.status_code}")
            return []
        ...
        if speakers_response.status_code != 200:
            print(f"스피커 조회 실패: {speakers_response.status_code}")
            return []
        
        speakers = {s['id']: s['name'] for s in speakers_response.json()}
        
        # 3. 비디오 데이터 조회
        videos_response = requests.get(
            f"{SUPABASE_URL}/rest/v1/influencer_videos",
            headers=headers,
            params={'select': 'id,title,subtitle_text'}
        )
        
        if videos_response.status_code != 200:
            print(f"비디오 조회 실패: {videos_response.status_code}")
            return []
        
        videos = {v['id']: v for v in videos_response.json()}
        ...
        for signal in signals:
            ...
            enriched_signal['speaker_name'] = speakers.get(speaker_id, '')
            
            # 비디오 정보 추가
            video_id = signal.get('video_id')
            video = videos.get(video_id, {})
            enriched_signal['video_title'] = video.get('title', '')
            enriched_signal['subtitle_text'] = video.get('subtitle_text', '')
            
            enriched_signals.append(enriched_signal)
        
        print(f"총 {len(enriched_signals)}개 시그널 조회 완료")
...
def check_key_quote_quality(signal_id, stock, ticker, key_quote, signal_type):
    ...
    if not key_quote or len(key_quote.strip()) < 20:
        issues.append({
            ...
            'description': f'key_quote가 20자 미만 (현재 {len(key_quote)}자): "{key_quote}"',
            'stock': stock,
            'signal': signal_type
        })
...
"""
    ...
    # key_quote 품질 이슈 분석
    quote_issues = issues['key_quote_quality']
    if quote_issues:
        short_quotes = [i for i in quote_issues if i['type'] == 'key_quote_too_short']
        no_stock = [i for i in quote_issues if i['type'] == 'stock_not_mentioned_in_quote']
        no_reasoning = [i for i in quote_issues if i['type'] == 'no_investment_reasoning']
        
        report += f"- key_quote 너무 짧음: {len(short_quotes)}건\n"
        report += f"- 종목명 미포함: {len(no_stock)}건\n"
        report += f"- 투자근거 없음: {len(no_reasoning)}건\n"
...
def main():
    ...
    # 보고서 저장
    total_issues = save_analysis_report(2, issues, len(signals))
    ...
    return issues, len(signals), total_issues
