...
### 파이프라인 규칙 최종 확정 (14:10~14:53)
1. **1영상 1종목 1시그널** — 맥락 종합해서 1개만 (뻥튀기 금지)
2. **타임스탬프 필수** — 영상 링크에 발언 시점 포함 (&t=초)
3. **GPT-4o 2차 검증 제거** — $14 써서 "전부 통과" 판정, Claude Haiku $0.05가 90개 수정 잡음. GPT-4o는 돈낭비
4. **파이프라인 2단계로 축소**: GPT-mini(추출 $0.07) → Claude Sonnet(검증 $1~2) → 사람
5. **Claude corrected 의견** — 오류 발견 시 수정 의견 제시
6. **리뷰 페이지**: 1차(GPT-mini추출) → 2차(Claude Sonnet검증+의견) → 3차(사람)
7. 리뷰 페이지는 전체 인플루언서용 (코린이아빠 전용 X), 인플루언서 필터 포함
8. 종목 옆에 유튜버 이름 표시, 최신순 정렬, 한글 판정명
...
### Claude 전체 검증 완료 (14:30~14:39)
- 164개 시그널 100% Claude 검증 완료
- confirmed 57 (35%), corrected 90 (55%), rejected 6 (4%), error 11 (7%)
- corrected 55% → Claude가 GPT 추출을 꽤 많이 수정함 (파이프라인 가치 입증)
- 리뷰 페이지 최종 빌드 완료 (4단계, 오프라인, 데이터 내장)
- git tag: pipeline-v2-claude-verified

### 애널 탭 화면 구조 확정 (14:57)
- **메인** (종목 선택 전): 애널 랭킹 + 최신 리포트 피드 + 기타
- **종목 선택 후**: 주가 차트 + 마커 + 해당 종목 애널 랭킹 + 리포트 리스트
- 차트는 종목 검색했을 때만 보여야 함 (메인에서 바로 안 보임)

### 애널리스트 차트 v4 완성 (14:35~14:39)
- test-analyst-chart.html v4: 캔들스틱 차트 + 7가지 기능 전부
- lightweight-charts + 마커 + AI요약 + 리포트 모달 + 컨센서스밴드 + 적중률 랭킹 + 프로필

### signal-review.html 런타임 에러 (14:50~)
- 유저가 열었더니 "그것도 오류" 보고
- EMBEDDED_SIGNALS_DATA 배열은 정상 존재 (194개)
- JS 렌더링/처리 로직에서 런타임 에러 발생 추정
- 필드 매핑 이슈: 새 파이프라인은 asset/signal_type/content/context, 기존 HTML은 stock/signal/quote/detail 기대
- _fix_review2.js로 매핑 스크립트 만들었으나 아직 미해결

### TODO
- 유저 피드백 대기 (애널 차트 v4 + 리뷰 페이지)
- 코린이 아빠 시그널 프로토타입(guru_tracker) 반영 (164개)
- 증권사 리포트 크롤러 개발 (네이버 금융)
- 터널 안정화
