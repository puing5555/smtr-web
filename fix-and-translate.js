...
// First fix the broken JSON: find lines with videoTitle that have nested quote issues
// Pattern: "videoTitle": ""text"",  -> needs to be "videoTitle": "text",
c = c.replace(/"videoTitle": ""/g, '"videoTitle": "');
// Fix trailing double quotes before comma
c = c.replace(/"videoTitle": "(.*)"",\n/g, (m, v) => `"videoTitle": "${v}",\n`);
...
let signals;
try {
  signals = JSON.parse(arrStr.substring(0, lastBracket + 1));
} catch(e) {
  console.log('Parse error:', e.message);
  // Find the problematic line
  const lines = arrStr.substring(0, lastBracket + 1).split('\n');
...
// Also catch partial matches for titles that got truncated
const partialMap = {};
for (const [k, v] of Object.entries(map)) {
  partialMap[k.substring(0, 40)] = { full: k, translation: v };
}
...
for (const s of signals) {
  ...
  if (map[s.videoTitle]) {
    ...
    // Try partial match
    const prefix = s.videoTitle.substring(0, 40);
    if (partialMap[prefix]) {
      s.videoTitle = partialMap[prefix].translation;
      count++;
    }
  }
}
...
for (const s of signals) {
  ...
  if (ascii > 0.5) {
    rem++;
    console.log('STILL ENG:', s.videoTitle.substring(0, 80));
  }
}
console.log('Remaining English:', rem);
