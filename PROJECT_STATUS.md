...
### ✅ 완료 - **🔍 V7 프롬프트 고도화 - ETF/해외/미국주식 채널 그룹 분석** (2026-02-27)
- **분석 대상**: 소수몽키(ETF/해외주식), 미주은(미국주식), 머니하이(해외주식/ETF) 3개 채널
- **제약사항**: web_search API 키 없음, 유튜브 동적 콘텐츠 접근 제한으로 이론적 분석 수행
- **발견한 문제 패턴 2개**:
  1. **ADR vs 현지주식 구분 부재**: "테슬라 ADR 팔고 나스닥 직접투자" 등에서 market=US_ADR 세분화 필요
  2. **ETF NAV 차익거래 vs 투자의견 혼동**: NAV 할인 차익거래를 일반 투자의견과 구분 필요, mention_type="차익거래" 추가 제안
- **적절히 처리되는 패턴들**: 환율 조건부 추천, 복합 ETF 대체, 해외 지수 배분 전략, 시간대별 조건부 등은 기존 V7 규칙으로 충분
- **결과 저장**: `C:\Users\Mario\work\prompts\v7_issues_group_b.md` 완성
