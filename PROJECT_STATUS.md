...
- **DB**: Supabase (31개 시그널)
  - 삼프로TV V7 20개 시그널 
  - 코린이아빠 V5 11개 시그널
- **시그널 타입**: 매수/긍정/중립/부정/매도 (한글 5단계만) ← 2026-03-05 경계→부정 변경완료
- **프론트엔드**: Next.js (56페이지)
...
### ✅ 애널리스트 탭 5가지 수정 완료 (2026-03-02 00:00) - GitHub Pages 배포
1. **✅ 증권사 컬럼 제거 + 리포트 상세 모달 추가**
   - 최신 리포트 탭에서 증권사 표시 제거
   - 카드 클릭 시 리포트 상세 모달 표시 (제목, 종목, 증권사, 애널리스트, 목표가, 투자의견, PDF링크, AI요약)
2. **✅ 날짜 형식 수정**: "2026-02-27" → "26.02.27" 짧은 형식으로 변경
3. **✅ AI 한줄요약 생성**: 567개 리포트 전체에 `summary` 필드 추가
   - 프로그래밍적 템플릿 기반 요약 생성
   - 제목 키워드 + 증권사 + 종목명 + 목표가 + 투자의견 조합
   - 예시: "SK증권이 원익QnC에 BUY 의견, 목표가 14만원 제시. 성장성 기대"
4. **✅ 차트 추가**: 종목별 탭에서 목표가 히스토리 바 차트 구현
   - `components/TargetPriceChart.tsx` 신규 생성
   - 증권사별 최신 목표가 비교 차트
   - 평균 목표가 및 목표가 범위 표시
5. **✅ 애널리스트명 수정**: null 애널리스트명 일부 업데이트 (2개/567개)
   - PDF URL 패턴 분석 및 ID 추출 로직 구현
   - 네이버 연구 페이지 시뮬레이션 (실제 크롤링 대신)
