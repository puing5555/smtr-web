#!/usr/bin/env python3
"""
AI Detail 519건 단순 재생성 스크립트 (디버그 버전)
순차 처리 방식 (안전하고 확실함)
"""
...
def regenerate_single_report(ticker: str, report_index: int, report: dict) -> str:
    ...
    try:
        print(f"  🤖 Claude API 호출: {ticker}_{report_index}")
        ...
        print(f"  ✅ 성공: {ticker}_{report_index}")
        return result
...
def main():
    ...
    print(f"🔄 진행상황: {current_index}/{total_reports} (완료: {len(completed)}, 실패: {len(failed)})")
    ...
    for idx in range(current_index, total_reports):
        ...
        if report_id in completed:
            print(f"⏭️ 건너뛰기 (완료됨): {report_id}")
            continue
            
        print(f"\n📝 처리중 [{idx+1}/{total_reports}]: {report_id}")
        print(f"   제목: {report.get('title', '')[:50]}...")
        ...
        # 50건마다 그룹에 보고
        if (idx + 1) % 50 == 0:
            progress_msg = f"🤖 AI Detail 재생성 진행현황\n\n진행: {idx+1}/{total_reports} ({(idx+1)/total_reports*100:.1f}%)\n✅ 완료: {len(completed)}건\n❌ 실패: {len(failed)}건\n\n예상 완료: 약 {(total_reports - idx - 1) * 2 / 60:.0f}분 후"
            print(f"\n📊 50건 단위 보고:\n{progress_msg}")
    ...
    final_msg = f"🎉 AI Detail 519건 재생성 완료!\n\n📊 최종 결과:\n- 총 처리: {total_reports}개\n- ✅ 성공: {len(completed)}개\n- ❌ 실패: {len(failed)}개\n- 📁 출력 파일: {output_file}\n\n💰 예상 비용: 약 $4\n⏱️ 소요 시간: 약 17분"
