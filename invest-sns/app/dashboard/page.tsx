                ...
                return (
                  <div key={stock.id} className="flex justify-between items-center py-2 border-b border-[#f0f0f0] last:border-0">
                    ...
                    <div className="text-right">
                      <p className={`font-medium ${returnPct >= 0 ? 'text-[#f04452]' : 'text-[#3182f6]'}`}>
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm">
          ...
          {watchlist.length > 0 ? (
            ...
            <div className="text-center py-4 text-[#8b95a1] text-sm">
              <p>관심종목이 없습니다</p>
              <Link href="/my-watchlist" className="text-[#3182f6] text-sm mt-2 inline-block">
                관심종목 추가 →
              </Link>
            </div>
          )}
          ...
          {notifications.length > 0 ? (
            <div className="space-y-3">
              {notifications.map((notif) => (
                <div
                  key={notif.id}
                  className={`py-2 border-b border-[#f0f0f0] last:border-0 ${
                    !notif.is_read ? 'bg-blue-50 -mx-2 px-2 rounded-lg' : ''
                  }`}
                >
