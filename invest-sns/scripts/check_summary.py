...
res = sb.table('influencer_signals').select('id,stock_name,signal_type,key_quote,reasoning').execute()
data = res.data
...
# 비용 추정
# 입력: stock_name + signal_type + key_quote + reasoning ~200토큰
# 출력: 3-5문장 ~150토큰
# Claude Sonnet: $3/M input, $15/M output
input_tokens = len(target) * 200
output_tokens = len(target) * 150
...
print(f'  API 호출: {len(target)}회')

if short:
    s = short[0]
    print(f'\n짧은 예시: {s["stock_name"]} / {s["signal_type"]}')
    print(f'  key_quote: {s.get("key_quote","")}')
