        ...
        try {
            ...
            const { data: signal, error } = await supabase
                ...
                .select(`
                    *,
                    speakers(name),
                    influencer_videos(id, title, published_at)
                `)
                .eq('id', signalId)
                .single();
            ...
            groups[key].signals.push({
                id: signal.id,
                stock: signal.stock,
                signal_type: signal.signal, // DB에서는 'signal' 컬럼 사용
...
function isDuplicate(signal1, signal2) {
    ...
    return similarity > 0.3; // 30% 이상 겹치면 중복으로 판단
}
...
async function consolidateBasketSignals(group) {
    ...
    signals.forEach(s => {
        console.log(`  - ${s.stock}: ${s.key_quote.substring(0, 30)}...`);
    });
    ...
    for (let i = 0; i < signals.length; i++) {
        ...
        for (let j = 0; j < consolidated.length; j++) {
            if (isDuplicate(signals[i], consolidated[j])) {
                console.log(`    🔄 중복 발견: ${signals[i].stock} ↔ ${consolidated[j].stock}`);
    ...
    console.log(`  → 중복 제거 후: ${consolidated.length}개`);
    ...
    if (consolidated.length >= 3) {
        ...
        console.log(`  시간 범위: ${timeSpread}초`);
        
        if (timeSpread <= 120) { // 2분 이내
            ...
            if (foundSemis.length >= 3) {
                ...
                // 베이스 시그널을 섹터로 업데이트
                const sectorQuote = `반도체 섹터 포트폴리오 추천: ${foundSemis.map(s => s.stock).join(', ')} 등 주요 반도체 종목들에 대한 집중 투자 전략을 제시했습니다.`;
...
async function main() {
    console.log('🚀 V10.1 바스켓 중복 통합 시작 (v2)');
    ...
    for (const [groupKey, group] of Object.entries(groups)) {
        ...
        for (const deleteId of consolidation.deleteSignals) {
            const deleteSuccess = await deleteSignal(deleteId);
            if (deleteSuccess) {
    ...
    // 결과를 파일로 저장
    await fs.writeFile(
        'scripts/v10_basket_fix_results_v2.json',
        JSON.stringify(results, null, 2)
    );

    console.log('\n결과가 scripts/v10_basket_fix_results_v2.json에 저장되었습니다.');
}
