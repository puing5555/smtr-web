...
def deduplicate_signals(signals: List[Dict]) -> List[Dict]:
    ...
    print(f"✅ 중복제거 완료: {merge_count}개 시그널 합병")
    print(f"📊 최종 시그널 수: {len(deduplicated)}개")
    
    return deduplicated

def save_signals_json(signals: List[Dict]) -> None:
    """시그널을 JSON 파일로 저장"""
    output_data = {
...
def update_signal_prices_json(signals: List[Dict]) -> None:
    """signal_prices.json 업데이트"""
    try:
        # 기존 파일 읽기
        ...
        print(f"📈 signal_prices.json 업데이트 완료: {len(new_stocks)}개 신규 종목 추가")
        ...
        print(f"❌ signal_prices.json 업데이트 실패: {e}")

def upload_to_supabase(signals: List[Dict]) -> None:
    """Supabase에 시그널 업로드"""
    print("\n🚀 Supabase 업로드 준비 중...")
    
    # TODO: Supabase 업로드 로직 구현
    # 현재는 JSON 파일만 생성
    upload_data = []
    ...
    print(f"📤 Supabase 업로드 데이터 준비 완료: {len(upload_data)}개 레코드")
    print("ℹ️  sesang101_supabase_upload.json 파일을 확인하여 수동 업로드하세요")

def main():
    """메인 실행 함수"""
    print("[INFO] 세상학개론 시그널 통합 및 중복제거 시작\n")
    ...
    # 5. Supabase 업로드 준비
    upload_to_supabase(deduplicated_signals)
    
    print(f"\n✅ 처리 완료!")
    print(f"📊 원본 시그널: {len(all_signals)}개")
    print(f"📊 중복제거 후: {len(deduplicated_signals)}개")
    print(f"📉 중복제거된 수: {len(all_signals) - len(deduplicated_signals)}개")
