...
FALSE_SIGNAL_RULES = """
★ 오추출 방지 규칙 (v2 - Opus 검증 결과 반영):

[1. 일반논평/교육적 비교 → 시그널 아님]
- 교육/분석 맥락에서 종목 간 단순 비교("~에 비해 상대적으로 안전하다", "~보다 구조가 낫다")는 투자 추천이 아닙니다
- 특히 "상대적으로", "비교하면", "~에 비해" 등 비교 표현이 포함된 중립적 분석은 시그널로 추출하지 마세요
- 투자 추천 의도가 명확하지 않은 일반론/구조 분석은 시그널이 아닙니다

[2. 조건부/한정 대상 조언 → 시그널 아님]
- "~하면", "~할 때", "나중에", "수익이 나면" 등 조건부 표현이 포함된 투자 언급은 시그널로 추출하지 마세요
- "불안하신 분들", "걱정하시는 분들", "리스크로 보는 분들" 등 특정 대상에 한정된 조언은 시그널이 아닙니다
- 개인적 투자 계획이나 막연한 의향 표현("언젠가 사고 싶다", "나중에 매집할 거다")은 시그널이 아닙니다
- 구체적인 매수/매도 근거나 타이밍이 없는 막연한 투자 의향은 제외

[3. 자막 원문 검증 (할루시네이션 방지 — 가장 중요)]
- ★ 시그널 추출 시 반드시 해당 종목명이 자막 텍스트에 실제로 존재하는지 확인하세요
- ★ 자막에 언급되지 않은 종목에 대해 시그널을 생성하지 마세요
- ★ timestamp는 반드시 자막에 존재하는 타임스탬프 범위 내에서만 사용하세요
- ★ content에 인용한 문장이 자막에 실제로 존재하는지 확인하세요
- 자막에서 찾을 수 없는 발언을 만들어내면 절대 안 됩니다
- 의심스러우면 추출하지 마세요. 없는 것을 만드는 것보다 놓치는 게 낫습니다
"""
