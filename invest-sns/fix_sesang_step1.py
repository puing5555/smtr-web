...
def get_youtube_title(video_id):
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            print(f"✓ 제목 획득: {video_id} -> {title}")
            return title
        else:
            print(f"✗ 제목 오류 {video_id}: {result.stderr}")
            return None
    except subprocess.TimeoutExpired:
        print(f"✗ 제목 타임아웃: {video_id}")
        return None
    except Exception as e:
        print(f"✗ 제목 예외 {video_id}: {e}")
        return None
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            if len(date_str) == 8 and date_str.isdigit():
                ...
                print(f"✓ 날짜 획득: {video_id} -> {formatted_date}")
                return formatted_date
            else:
                print(f"✗ 날짜 형식 오류: {video_id} -> {date_str}")
                return None
        else:
            print(f"✗ 날짜 오류 {video_id}: {result.stderr}")
            return None
    except subprocess.TimeoutExpired:
        print(f"✗ 날짜 타임아웃: {video_id}")
        return None
    except Exception as e:
        print(f"✗ 날짜 예외 {video_id}: {e}")
        return None
...
def update_video(video_id, title=None, published_at=None):
    ...
    if response.status_code == 204:
        ...
        print(f"✓ 업데이트 완료: {video_id} ({', '.join(update_fields)})")
        return True
    else:
        print(f"✗ 업데이트 실패: {video_id} -> {response.status_code}")
        return False
...
def main():
    ...
    for i, video in enumerate(videos):
        ...
        if new_title or new_date:
            ...
            print(f"✗ 업데이트할 데이터 없음: {video['video_id']}")
            error_count += 1
