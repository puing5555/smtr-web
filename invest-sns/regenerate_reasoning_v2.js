...
async function callAnthropicAPI(prompt, timeoutMs = 30000) {
  ...
  try {
    ...
    if (error.name === 'AbortError') {
      throw new Error('API 요청 타임아웃 (30초)')
    }
    throw error
  }
}
...
async function main() {
  ...
  for (let i = 0; i < filteredSignals.length; i += 5) {
    ...
    console.log(`\n📦 배치 ${Math.floor(i/5) + 1}/${Math.ceil(filteredSignals.length/5)} 처리 중... (${batch.length}개)`)
    ...
    for (const signal of batch) {
      try {
        ...
        if (!newReasoning || newReasoning.length < 80) {
          console.log(`    ⚠️  생성된 reasoning이 너무 짧음: ${newReasoning?.length || 0}자`)
          failureCount++
        ...
        if (updateError) {
          ...
          console.log(`    ✅ 성공 (${newReasoning.length}자): ${newReasoning.substring(0, 50)}...`)
          successCount++
        }
  ...
  if (failedSignals.length > 0) {
    ...
    if (failedSignals.length > 10) {
      console.log(`  ... 그리고 ${failedSignals.length - 10}개 더`)
    }
  }
