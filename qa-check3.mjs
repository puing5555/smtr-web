...
async function run(){
  ...
  let r=await fetch(BASE+'/rest/v1/influencer_videos?select=id,video_id,title,published_at&id=in.('+q+')',{headers:h});
  let vids=await r.json();
  if(Array.isArray(vids)) vids.forEach(v=>console.log(v.id,'|',v.video_id,'|',v.title?.substring(0,40),'|',v.published_at));
  else console.log(JSON.stringify(vids));
}
run();
